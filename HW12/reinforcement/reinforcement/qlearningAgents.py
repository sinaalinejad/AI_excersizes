...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        if len(legalActions) == 0:
            return 0.0
        max_action = self.Qvalues[(state, legalActions[0])]
        for action in legalActions:
            if self.Qvalues[(state, action)] > max_action:
                max_action = self.Qvalues[(state, action)]
        return max_action
    ...
    def computeActionFromQValues(self, state):
        ...
        for action in legalActions:
            if self.Qvalues[(state, action)] > self.Qvalues[(state, max_action)]:
                max_action = action
        return max_action
